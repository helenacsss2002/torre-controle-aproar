...
# ---------------------------------------------------------------------------
# PRAZOS DE APONTAMENTO E COBRANÇAS TEAMS
# ---------------------------------------------------------------------------
# Demais unidades:
#   D 16:00 = 1ª cobrança e passa a ser atraso;
#   D+1 09:30 = 2ª cobrança;
#   D+1 15:00 = 3ª e última automática.
#
# SEBRAE:
#   jornada 17h–02h = diária integral;
#   adicional noturno padrão = R$ 90,00;
#   automática ÚNICA às 21:00 do próprio dia;
#   pode ser apontado na madrugada/manhã de D+1;
#   só vira atraso operacional às 09:30 de D+1;
#   depois permanece disponível para cobrança manual.
VALOR_ADICIONAL_NOTURNO_SEBRAE = 90.00

HORA_ATRASO_GERAL = datetime.time(16, 0)
HORA_COBRANCA_GERAL_DIA = datetime.time(16, 0)
HORA_COBRANCA_DIA_SEGUINTE_1 = datetime.time(9, 30)
HORA_COBRANCA_DIA_SEGUINTE_2 = datetime.time(15, 0)

HORA_COBRANCA_SEBRAE = datetime.time(21, 0)
...
def apontamento_esta_atrasado(
    ...
    if agora.date() == data_servico:
        return agora.time() >= HORA_ATRASO_GERAL
...
def situacao_pendencia_teams(
    ...
    if eh_unidade_sebrae(unidade):
        if data_servico == agora.date():
            if agora.time() < HORA_COBRANCA_SEBRAE:
                return "Pendente · lembrete automático 21:00"
            return "Pendente SEBRAE · lembrete 21:00 já devido"
        ...
        if (
            agora.date() == dia_seguinte
            and agora.time()
            < HORA_LIMITE_APONTAMENTO_SEBRAE
        ):
            return "Pendente SEBRAE · dentro da janela de apontamento"

        return "ATRASADO · somente cobrança manual"

    if data_servico == agora.date():
        if agora.time() < HORA_ATRASO_GERAL:
            return "Pendente · automática às 16:00"
    ...
    if agora.date() == dia_seguinte:
        if agora.time() < HORA_COBRANCA_DIA_SEGUINTE_1:
            return "ATRASADO · automáticas 09:30 e 15:00"
        if agora.time() < HORA_COBRANCA_DIA_SEGUINTE_2:
            return "ATRASADO · próxima automática 15:00"
        return "ATRASADO · última automática 15:00 / manual disponível"

    return "ATRASADO · somente cobrança manual"


def proxima_automacao_pendencia(
    data_servico,
    unidade="",
    agora=None,
):
    ...
    if eh_unidade_sebrae(unidade):
        if data_servico == agora.date():
            if agora.time() < HORA_COBRANCA_SEBRAE:
                return "Hoje 21:00"
            return "21:00 · única automática"
        return "Somente manual"

    if data_servico == agora.date():
        if agora.time() < HORA_COBRANCA_GERAL_DIA:
            return "Hoje 16:00"
        return "16:00 · depois D+1 09:30 e 15:00"
...
def _carregar_pendentes_responsavel_teams(
    ...
    """
    Retorna TODAS as pendências abertas até hoje.

    Esta função alimenta:
      - prévia da Configuração;
      - botão manual "Cobrar agora".

    A agenda automática é decidida pelo script cobrar_apontamentos_teams.py.
    """
    supervisor = str(
        supervisor
        or ""
    ...
    if supervisor in OBSERVADORES_TEAMS:
        cur.execute(
            ...
            ORDER BY c.data, o.unidade, col.nome
            """,
            (
                hoje_ref,
                "A DEFINIR NO APONTAMENTO%",
...
def incluir_multiplos_servicos_direto_apontamento(
    ...
    pagamentos_por_turno = dict(
        pagamentos_por_turno
        or {}
    )
    ...
    colab_ref_pag = (
        ...
        if "dict_colaboradores" in globals()
        else {}
    )
    ...
    for turno_novo, itens_turno in grupos_turno.items():
        ...
        pagamento_turno = dict(
            pagamentos_por_turno.get(
                turno_novo,
                {},
            )
            or {}
        ...
        if tem_sebrae_turno:
            tipo_diaria_turno = "Diária"

        diaria_fin_turno = float(
            pagamento_turno.get(
                "diaria"
            )
            or valor_financeiro_padrao_colaborador(
                colab_ref_pag,
                tipo_diaria_turno,
            )
        )
        ...
        payload = {
            ...
            "valor_adicional_noturno": round(
                adicional_noturno_turno,
                2,
            ),
            "custo_encargos_base": round(
                custo_ctrl_turno,
                2,
            ),
            "custos_separados": True,
        ...
        grupos_payload.append({
            ...
            "tipo_diaria": tipo_diaria_turno,
            "diaria": diaria_fin_turno,
            "extra": extra_turno,
            "acordo": acordo_turno,
            "adicional_noturno": adicional_noturno_turno,
        })
...
if modo_login:
    ...
    if area_campo == "Hoje":
        ...
        with st.expander(
            ...
            for item_inc in servicos_inc_ui:
                turno_pag = normalizar_turno_convocacao(
                    item_inc.get("turno")
                    or "Integral"
                )

                if turno_pag not in turnos_pag_inc:
                    turnos_pag_inc.append(
                        turno_pag
                    )
            ...
            if turnos_pag_inc:
                ...
                st.caption(
                    "Diária e Extra ficam separados. Serviços do mesmo turno "
                    "compartilham uma única diária, que será rateada entre as obras."
                )

            for turno_pag in turnos_pag_inc:
                itens_turno_ui = [
                    x
                    for x in servicos_inc_ui
                    if normalizar_turno_convocacao(
                        x.get("turno")
                        or "Integral"
                    ) == turno_pag
                ]
                ...
                diaria_key_inc = (
                    f"engm_inc_diaria_{safe_turno}"
                )
                extra_key_inc = (
                    f"engm_inc_extra_{safe_turno}"
                )
                acordo_key_inc = (
                    f"engm_inc_acordo_{safe_turno}"
                )
                noturno_key_inc = (
                    f"engm_inc_noturno_{safe_turno}"
                )

                if diaria_key_inc not in st.session_state:
                    st.session_state[
                        diaria_key_inc
                    ] = valor_financeiro_padrao_colaborador(
                        colab_inc_ref,
                ...
                if extra_key_inc not in st.session_state:
                    st.session_state[
                        extra_key_inc
                    ] = 0.0

                if acordo_key_inc not in st.session_state:
                    st.session_state[
                        acordo_key_inc
                    ] = 0.0
                ...
                with st.container(
                    border=True,
                    key=f"engm_inc_pag_{safe_turno}",
                ):
                    st.markdown(
                        f"**{turno_pag} · {tipo_diaria_pag}**"
                    )
                        ...
                        with st.spinner(
                            ...
                            ok, msg = (
                                incluir_multiplos_servicos_direto_apontamento(
                                    ...
                                    pagamentos_por_turno=pagamentos_inc_ui,
                                )
                            )
    ...
    # --- 2. CONVOCAÇÃO ---
        ...
        cabecalho_pagina_aproar(
            "Convocação",
            "Monte a equipe para a data necessária. Paulo/Admin também pode fazer convocação retroativa para correções operacionais.",
            categoria="OPERAÇÃO",
        )
        ...
        with tab_nova_conv:
            if obras:
                ...
                data_conv_auto = st.date_input(
                    ...
                    key="data_conv_adm",
                    help=(
                        "Paulo/Admin pode selecionar datas passadas para "
                        "convocação retroativa ou correção operacional."
                    ),
                )
                ...
                with col_info:
                    ...
                    if data_conv_auto < hoje_admin:
                        st.warning(
                            "Convocação retroativa liberada para Paulo/Admin · "
                            f"{data_conv_auto.strftime('%d/%m/%Y')}"
                        )
                    elif data_conv_auto == hoje_admin:
                        st.info(
                            f"📅 **Hoje:** {data_conv_auto.strftime('%d/%m/%Y')}"
                        )
                    else:
                        st.info(
                            f"📅 **Data selecionada:** {data_conv_auto.strftime('%d/%m/%Y')}"
                        )
                with col_turno:
    ...
    # --- 7. CONFIGURAÇÕES E SINCRONIZAÇÃO TRELLO ---
        ...
        with tab_teams:
            ...
            if DB_BACKEND != "NEON":
                ...
                st.caption(
                    "Esta prévia mostra toda pendência aberta, mesmo antes do horário automático. O envio manual continua disponível enquanto a pendência existir."
                )
                ...
                try:
                    ...
                    for item in pendentes_preview:
                        ...
                        linhas_preview.append({
                            ...
                            "Próxima automática": proxima_automacao_pendencia(
                                data_ref,
                                unidade=unidade_ref,
                                agora=agora_preview,
                            ),
                ...
                st.caption(
                    ...
                    "09:30 e 15:00 no dia seguinte. SEBRAE: uma única automática "
                    "às 21:00 no próprio dia. Toda pendência continua visível para "
                    "cobrança manual enquanto não for apontada."
                )
